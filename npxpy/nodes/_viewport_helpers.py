...
class _meshbuilder:
    ...
    def ca_mesh(coarse_aligner):
        ...
        for anchor_i in coarse_aligner.alignment_anchors:
            ...
            label = pv.Text3D(
                f"{label_dummy_label}", height=10, depth=0, center=(0, -120, 0)
            ) + pv.Text3D(
        ...
        def get_smallest_radius_at_z0():
            ...
            for phi in range(nr_phi_segments):
                for r in range(nr_radial_segments):
                    current_point = surf.points[(nr_radial_segments * phi) + r]
                    if (
                        current_point[2] < 0
    ...
    def ia_mesh(self, interface_aligner_node):
        ...
        cross_shape = self.make_cross(size=5.0)  # Size the cross as you like
